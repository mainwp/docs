---
title: What Does Dashboard Insights Track?
sidebarTitle: "Insights Tracking"
description: >-
  Dashboard Insights summarizes activity recorded in the shared Network Activity
  logs for your MainWP Dashboard and child sites. This reference lists the
  available event types.
---

Dashboard Insights provides analytics data about your MainWP Dashboard and child site usage. This page lists the event types available to the shared Network Activity logs dataset.

## What You'll Learn

- Event types available when actions originate from MainWP Dashboard
- Event types available when changes happen directly on child sites (Non-MainWP Changes)

<Info>
Dashboard Insights only includes events recorded by Network Activity. In **MainWP > Settings > Network Activity Settings**, turn on **Enable Network Activity logging** and use **Select events to log** to choose which event types generate new entries. Unchecked event types do not generate new log entries.
</Info>

Network Activity uses available context to show recognizable object titles, including plugin and theme names for synchronized updates and affected table names for database changes. It also uses child-site activity context to identify the responsible user when possible. Theme updates performed directly in child site WP Admin are included as **Theme Updated** events.

To reduce noisy logs, **Opened a post in editor**, **Viewed a post**, and **Opened the profile page of a user** are disabled by default. Enable any of these events under **Select events to log** if you need them.

---

## Events from MainWP Dashboard

These event types can be recorded when actions are performed through your MainWP Dashboard.
...
These event types can be recorded when changes happen directly on child sites outside of MainWP Dashboard.
...
- [Dashboard Insights](/dashboard/overview/dashboard-insights) - Insights overview
- [Network Activity](/sites/network-activity#view-only-non-mainwp-changes) - View Non-MainWP Changes
