...
MainWP integrates with popular backup solutions through add-ons and API-based backups. Use backup add-ons for plugin-based solutions, or API Backups to trigger backups directly through your hosting provider or cloud manager.

## What You'll Accomplish

- Configure backup add-ons for plugin-based backups
...
- Backup add-on installed, or API credentials from your hosting provider
...
MainWP integrates with popular backup plugins through dedicated add-ons:
...
<iframe
  ...
  title="API Backups Walkthrough - Video Tutorial"
  frameBorder="0"
...
Enter the credentials required by your provider in the MainWP settings. For Cloudways, use an **Access Token**; existing connections can continue using the legacy **Account Email** and **API Key** fields.
...
<Warning>
For Plesk API Backups, enter only the Plesk panel URL in the **Plesk URL** field and do not add a trailing slash. Use `https://your-plesk-hostname:8443`, not `https://your-plesk-hostname:8443/`. Do not include `/api/`, paths, query strings, or fragments.
</Warning>
...
After configuration, the API Backups tab displays Kinsta restore points such as automatic daily and manual backups. These entries can be restored or deleted, but they are not downloadable ZIP files. Click **Refresh Available Backups** to update the lists, or **Backup Now** to create a new manual restore point in Kinsta.

![Kinsta API Backups tab](/images/dashboard/sites-backups-9b1a66.png)

<Info>
A Kinsta downloadable backup is a separate ZIP export containing the site's files and database. MainWP cannot create this export. To create one, go to **MyKinsta > Sites > your site > Backups > Download** and click **Create backup now**.

When Kinsta finishes creating the archive, return to MainWP and click **Refresh Available Backups**. MainWP retrieves existing downloadable backups separately and displays any active download link returned by Kinsta under **Downloadable Backups**.

See [Kinsta's downloadable backup documentation](https://kinsta.com/docs/wordpress-hosting/wordpress-backups/#downloadable-backups) for current creation limits and availability periods.
</Info>
...
- [Backup Add-ons](https://mainwp.com/mainwp-add-ons/add-on-category/backups/) - All backup add-ons
