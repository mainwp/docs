...
- Troubleshoot premium plugin and theme update failures
...
<a id="troubleshoot-premium-plugin-failures"></a>

## Troubleshoot Premium Plugin and Theme Failures

If only premium products fail, try updating the affected item separately and check the vendor's requirements on that child site.

| Issue | Solution |
| --- | --- |
| License or registration not activated | Activate it on the affected child site |
| License expired | Renew it with the vendor if required for downloads |
| Vendor account or API credentials missing | Reconnect the account or enter the required keys in the product's settings |
| Update server unreachable | Check the vendor's status and whether the child site's host can reach the download server |
| Additional compatibility needed | Check **MainWP > Settings > Premium Updates** and add the correct custom identifier if needed |

![Premium Updates licensing notice and custom identifier controls](/images/dashboard/premium-updates-overview-dark.svg)

MainWP Dashboard 6.2 includes built-in compatibility for many premium products. Adding an identifier helps detection and update requests; it does not activate a license or guarantee that the vendor will deliver the download. Check whether the update works directly in the child site's **Dashboard > Updates**, then sync MainWP again.

See [Premium Updates](/sites/updates/premium-updates), [Premium Plugin Updates Not Detected](/troubleshooting/premium-plugin-updates-not-detected), or [Premium Theme Updates Not Detected](/troubleshooting/premium-theme-updates-not-detected) for the relevant workflow.
...
- [ ] Premium plugin and theme licenses are active (if applicable)
...
- [Premium Plugin Updates Not Detected](/troubleshooting/premium-plugin-updates-not-detected) - Premium update issues
