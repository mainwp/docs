---
title: Add-on Endpoints
sidebarTitle: "Add-on Endpoints"
description: >-
  Route-level reference for MainWP add-on REST endpoints including Comments,
  SSL Monitor, Domain Monitor, Lighthouse, Pro Reports, and Time Tracker.
---

Add-on endpoints provide access to functionality from MainWP add-ons. These routes require each corresponding add-on to be installed and active on your MainWP Dashboard.
...
- Which add-on routes are available in MainWP REST API v2
- Which parameters each add-on endpoint accepts
...
| POST, PUT, PATCH | `/ssl-monitor/{id_or_domain}` | Trigger SSL check for one site | `id_or_domain` |
...
| POST, PUT, PATCH | `/domain-monitor/{id_or_domain}` | Trigger domain check for one site | `id_or_domain` |

### Lighthouse

| Method | Path | Purpose | Key Params |
|---|---|---|---|
| POST, PUT, PATCH | `/lighthouse/audit` | Run Lighthouse audits for all supported sites | - |
| POST, PUT, PATCH | `/lighthouse/audit/{id_or_domain}` | Run Lighthouse audit for one site | `id_or_domain` |
...
|---|---|---|---|
| `start_date` | string | No | Start date in `Y-m-d` format. When either date is missing, the range defaults to the last year |
| `end_date` | string | No | End date in `Y-m-d` format. When either date is missing, the range defaults to the last year |
| `action` | string | No | Activity action filter (route-specific) |
| `format` | string | No | Response format |
| `multi_tokens` | string | On `multi-tokens` only | Report template content whose tokens the Dashboard resolves, for example `[client.name]` |
...
| GET | `/pro-reports/{id_or_domain}/widgets` | Widget activity |
...
| GET | `/pro-reports/{id_or_domain}/pagespeed` | Page Speed activity |
...
| GET | `/pro-reports/{id_or_domain}/multi-tokens` | Resolved report tokens for a template |
...
- [Lighthouse Integration](/add-ons/monitoring/mainwp-lighthouse-extension)
