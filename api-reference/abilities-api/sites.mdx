...
curl -X POST -u 'admin:xxxx' \
  -H 'Content-Type: application/json' \
  -d '{"input":{"url":"https://newsite.example.com","admin":"admin","name":"New Site","uniqueid":"xxxxx"}}' \
...
| `adminpassword` | string | No | - | WordPress administrator password. Required when Password Authentication is enabled on the child site |
| `uniqueid` | string | No | - | Unique Security ID from MainWP Child. Required when Unique Security ID is enabled or Password Authentication is disabled |
| `ssl_verify` | boolean | No | true | Verify SSL certificate |

MainWP Child requires at least one connection authentication method. Provide `adminpassword`, `uniqueid`, or both according to the child site's MainWP Child settings.
...
```bash
curl -G -u 'admin:xxxx' \
  --data-urlencode 'input_json={"site_id_or_domain":1,"status":"active"}' \
...
```bash
curl -G -u 'admin:xxxx' \
  --data-urlencode 'input_json={"site_id_or_domain":1,"page":1,"per_page":50}' \
