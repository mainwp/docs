...
sidebarTitle: "Abilities API Overview"
description: >-
...
<Card title="MainWP MCP Server" icon="robot" href="/mcp-server/overview">
...
MainWP Add-ons can register additional abilities alongside the 62 Dashboard abilities. These appear only when the relevant Add-on is active.

| Add-on | Category | Abilities | Reference |
|--------|----------|-----------|-----------|
| Cache Control 5.3 or later | `mainwp-cache-control` | 8 | [Cache Control Abilities](/api-reference/abilities-api/cache-control) |
| Custom Dashboard 5.2 or later | `mainwp-custom-dashboard` | 4 | [Custom Dashboard Abilities](/api-reference/abilities-api/custom-dashboard) |
| Google Search Console 6.3 or later | `mainwp-google-search-console` | 10 | [Google Search Console Abilities](/api-reference/abilities-api/google-search-console) |

These Add-ons use the `mainwp/` namespace. Cache Control provides cache status reads, policy changes, Cloudflare credential validation, and single-site cache purging. Custom Dashboard provides snippet metadata, PHP validation, and previewed snippet replacements and removals. Google Search Console provides connection and property reads, site mappings, cached performance summaries, refresh settings, and background refresh jobs. Find their references in the **Add-on Abilities** navigation group.
...
```http
GET|POST|DELETE /wp-json/wp-abilities/v1/abilities/{name}/run
```

### HTTP Methods

Choose the method from the ability's `meta.annotations`:

- **GET** - Read-only abilities (`readonly: true`), including those that require input.
- **DELETE** - Non-read-only abilities with both `destructive: true` and `idempotent: true`.
- **POST** - All other write abilities, including destructive abilities marked `idempotent: false`.

For example, `mainwp/delete-tag-v1` uses DELETE, while `mainwp/delete-site-v1` and `mainwp/delete-client-v1` use POST. These methods follow [WordPress Abilities method validation](https://developer.wordpress.org/reference/classes/wp_rest_abilities_v1_run_controller/validate_request_method/); using the wrong method returns `rest_ability_invalid_method` with HTTP 405.

### Input Handling

MainWP Dashboard 6.2 supports the following input formats for `mainwp/*` ability run routes.
...
```bash
curl -G -u "admin:xxxx" \
  --data-urlencode 'input_json={"site_id_or_domain":1,"include_stats":true}' \
...
curl -X POST -u "admin:xxxx" \
  ...
  -d '{"input":{"site_ids_or_domains":[1]}}' \
  "https://your-dashboard.com/wp-json/wp-abilities/v1/abilities/mainwp/sync-sites-v1/run"
```

**DELETE requests**: Use an `application/json` body containing only an `input` object. This keeps operation inputs out of the URL. For example, preview a tag deletion:
...
After reviewing the preview, execute with `confirm: true` and `dry_run: false`. The DELETE body must not exceed 1 MiB (1,048,576 bytes). Do not combine a body with `input` or `input_json` query parameters. Existing bracket-style `input` query parameters remain supported when no body is sent, but the JSON body is preferred. `input_json` is a GET-only parameter.
...
| Code | HTTP | Description |
|------|------|-------------|
| `mainwp_abilities_invalid_input_transport` | 400 | Invalid JSON input format, conflicting input sources, or transport size limit exceeded |
