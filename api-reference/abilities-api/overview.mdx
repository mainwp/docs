...
Both Add-ons use the `mainwp/` namespace. Cache Control provides cache status reads, policy changes, Cloudflare credential validation, and single-site cache purging. Google Search Console provides connection and property reads, site mappings, cached performance summaries, refresh settings, and background refresh jobs. Find their references in the **Add-on Abilities** navigation group.
