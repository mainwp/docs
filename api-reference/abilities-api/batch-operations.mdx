...
sidebarTitle: "Batch Operations Abilities"
description: >-
...
```bash
curl -G -u "admin:xxxx" \
  --data-urlencode 'input_json={"job_id":"sync_abc123def456"}' \
...
if [ "$QUEUED" = "true" ] && [ -n "$JOB_ID" ]; then
  ...
  # 3. Poll for status
  while true; do
    STATUS=$(curl -s -G -u "admin:xxxx" \
      --data-urlencode "input_json=$(jq -cn --arg job_id "$JOB_ID" '{job_id: $job_id}')" \
      "https://your-dashboard.com/wp-json/wp-abilities/v1/abilities/mainwp/get-batch-job-status-v1/run")
