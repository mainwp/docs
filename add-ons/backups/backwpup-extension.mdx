...
- MainWP Dashboard with the BackWPup Add-on installed
- BackWPup plugin installed and activated on each child site you want to back up

<Note>
Starting with add-on version 5.2, the MainWP BackWPup Add-on requires PHP 8.1 or newer on the MainWP Dashboard server. If your Dashboard runs an older PHP version, upgrade PHP before updating the add-on. Contact your host if you are not sure how to change the PHP version.
</Note>
...
<Note>
When you run a global job with the **Backup now** action, the add-on checks every eligible child site for the job. If a site does not yet have the job mapped (for example, a site that was added after the job was created), the add-on creates the job on that child site first, then starts the backup. The mapping is only saved after the child site confirms a valid job ID, and any partial mapping is cleaned up if the child site does not respond correctly.
</Note>
...
<Note>
MainWP Child automatically detects whether the installed BackWPup version uses the current job API or a legacy job handler. No compatibility setting is required; starting a job, stopping it, and reporting its progress use the appropriate method automatically.
</Note>
