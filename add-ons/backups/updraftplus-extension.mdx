...
The MainWP UpdraftPlus Extension needs to be installed only on your MainWP Dashboard site. You can [install it as any other MainWP add-on.](/dashboard/overview/manage-extensions#install-add-ons)
...
pCloud is an [UpdraftPlus Premium storage option](https://teamupdraft.com/documentation/updraftplus/topics/backing-up/faqs/how-to-backup-with-updraftplus/). Configure the connection and destination folder in UpdraftPlus on each child site before using it for backups from MainWP Dashboard.

1. On the child site, open **UpdraftPlus > Settings**, select **pCloud** under **Choose your remote storage**, and follow the prompts to authorize the pCloud account.
2. Set the destination folder in the pCloud settings and save the changes. Use a separate folder for each site so its backups are easy to identify.
3. Run a test backup with remote storage enabled, then confirm that the backup files appear in the intended pCloud folder.

After setup, use [Backup Now options](#backup-now-options) to send a backup to the configured remote destination. If the site has multiple destinations, select pCloud for that backup.

### Connect UpdraftPlus Premium

When you use Bulk Connect for UpdraftPlus Premium, MainWP verifies the final Premium state instead of treating an account connection alone as a completed activation. Depending on the TeamUpdraft account and child site, you may see one of these outcomes:

- The account is connected and Premium is fully active.
- The account is connected, but a Premium purchase must be claimed or assigned to the site.
- A purchase is assigned, but UpdraftPlus or its Premium add-ons must be updated or activated.
- No active Premium purchase is available for the site.
- UpdraftPlus could not verify the TeamUpdraft account connection.

When MainWP reports that action is required, open UpdraftPlus on the child site and follow its claim, assignment, update, or activation prompt. MainWP Child also prevents the WordPress.org UpdraftPlus package from replacing an installation where Premium add-ons are detected.
...
## MainWP UpdraftPlus Extension Backup/Restore
...
**You can get to the UpdraftPlus Backups tab from various places. On the add-on settings page, in all provided tables, you will find the Backup Now link. It will lead you directly to the child site backup page.**
