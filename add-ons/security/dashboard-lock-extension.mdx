...
Dashboard Lock 5.2.0 includes abilities that let you view the stored Dashboard Lock configuration and the application state of its extension-owned files. You can manage frontend redirects, IP restrictions, and HTTP Basic Authentication, check whether the Dashboard is ready to apply changes, and remove access locks for recovery.

Calls must authenticate through the WordPress REST API. Reading the general status requires access to the Dashboard Lock extension. Reading stored configuration or changing a lock requires a WordPress user with `manage_options` and extension access. A MainWP API key must also grant the required REST permissions.

The redirect, IP restriction, HTTP Basic Authentication, and lock-removal abilities are destructive. They require the matching confirmation field set to literal `true`: `confirm_redirect_risk`, `confirm_lockout_risk`, or `confirm_recovery_action`.
...
<Warning>
Currently, the **Admin area lock** and **Login page lock** options do not work on nginx servers. MainWP is looking into adding nginx support for these locks.
</Warning>
