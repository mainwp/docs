...
## Manage Firewall and AutoCleanup

For connected sites on a paid Virusdie account, the dashboard shows **Firewall** and **AutoCleanup** columns with **Enabled** or **Disabled** statuses.

- **Firewall** controls Virusdie's website firewall, which helps block malicious requests.
- **AutoCleanup** controls whether Virusdie attempts to clean infected files automatically when it detects them. See [Virusdie's automatic cleanup explanation](https://virusdie.com/faq/antivirus/) for details.

Go to **MainWP > Add-ons > Security > Virusdie**, locate the site, and open its row action menu. Select **Enable Firewall** or **Disable Firewall**, or **Enable AutoCleanup** or **Disable AutoCleanup**, as needed. Each action changes only the selected setting for that site. Feature availability depends on the site's Virusdie plan and API permissions.
