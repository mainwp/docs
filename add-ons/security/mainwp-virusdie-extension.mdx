...
If you need to disconnect the extension from your Virusdie account or reset stored credentials (for example, before connecting a different account or to recover from a failed sign-in), use the **Clear Connection Data** button.

When to use it:

-   You want to switch to a different Virusdie account
-   The extension is stuck with stale or invalid API credentials
-   You're following the recovery steps in [the sync file error guide](/add-ons/security/mainwp-virusdie-extension#error-unique-sync-file-could-not-be-uploaded%E2%80%9D-when-adding-a-website-to-virusdie) and need to remove existing credentials before signing in again

To clear stored connection data:
...
<Step title="Click the Clear Connection Data button">
The button is located in the **Create a free Virusdie account** section at the top of the Settings tab. Clicking it signs the extension out of Virusdie and removes the stored email, API key, and HMAC key from your MainWP Dashboard.
</Step>
...
<Warning>
Clearing connection data only removes credentials stored in your MainWP Dashboard. It does not delete your Virusdie account or remove any sites already connected to it.
</Warning>
...
1.  In the Virusdie extension settings, use the Sign Out function (or the [Clear Connection Data](/add-ons/security/mainwp-virusdie-extension#clear-connection-data) button) to remove current credentials
