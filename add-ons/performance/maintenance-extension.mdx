...
<Warning>
Preview every destructive request before approval. For schedule operations, send `dry_run: true`. For an immediate site cleanup, call `mainwp/preview-site-maintenance-v1` and use its preview token. After the user approves the exact effect, send the destructive request with `confirm: true`. This applies to immediate cleanup runs and to schedule changes that can start cleanup later. Keep the recent database backup from the prerequisites available for recovery.
</Warning>
