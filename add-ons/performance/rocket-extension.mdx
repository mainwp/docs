...
- With WP Rocket abilities, you can read settings and site status from connected child sites, then run cache and settings actions.
- You receive a preview before you confirm cache or settings changes.
- Each action keeps a durable receipt and lease while it runs, so you can follow its pending, completed, or failed status.
- The integration accepts supported child-site response envelopes and verifies the required capability before you run an action.
