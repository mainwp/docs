...
- Protecting against concurrent snippet saves
...
If someone else saves the same snippet while you're editing, your save is rejected with a conflict message and your draft stays in the editor until you reload. This prevents two people from silently overwriting each other's changes.

If the canonical read of a snippet fails (for example, the stored option is corrupt), the editor renders a red error in place of the textarea and disables the **Save** button. Fix the underlying option or reload to try again.

---

## Programmatic access with the Abilities API

Starting in 5.2.0, the extension registers four abilities in the WordPress Abilities API under the `mainwp-custom-dashboard` category:

- `mainwp/get-custom-dashboard-snippet-metadata-v1` — reports whether a snippet is configured and returns its revision hash. It never returns snippet source.
- `mainwp/validate-custom-dashboard-php-v1` — checks tagless PHP for syntax and disallowed constructs without storing or running it.
- `mainwp/replace-custom-dashboard-snippet-v1` — previews or writes a snippet, guarded by an expected revision.
- `mainwp/clear-custom-dashboard-snippet-v1` — previews or clears a snippet, guarded by an expected revision.

Destructive abilities require `dry_run: false` and `confirm: true` together, and every write is bound to the revision returned by the metadata ability so concurrent changes are rejected with `mainwp_custom_dashboard_snippet_conflict`.

See [Custom Dashboard Abilities](/api-reference/abilities-api/custom-dashboard) for endpoint details, input and output schemas, and full examples.
