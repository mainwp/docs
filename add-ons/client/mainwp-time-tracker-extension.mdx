...
Version 5.2.0 adds 12 Time Tracker abilities for authorized automation tools. They can manage buckets, tasks, and timers, read reports, and keep track of a request.

Each operation confirms that the caller can access the exact bucket, task, timer, client, or site involved. Time entries, timer state, and billing totals update together. A running timer stays tied to its current generation, so pausing or recovering it produces the same reconciled state.

Task counters on Sites and Clients reflect the tasks for the respective site or client. Requests use the same request ID across responses, and invalid flags or unauthorized attempts return specific errors.
