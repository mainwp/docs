...
- Team Control abilities support role, member, permission, and role-note management.
- Protected authority and operation permissions are limited to authorized roles.
- Role changes use recoverable generational transactions, and changes require confirmation before they are applied.
- Existing installations seed an owner automatically, and users cannot grant themselves protected access.
- User removal verifies the multisite deletion result before changing role membership.
- Repeating a role-note request does not create duplicates, and a complete role update replaces the prior permission set.
