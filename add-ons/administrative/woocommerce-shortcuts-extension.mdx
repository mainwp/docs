...
Version 5.1.0 adds a WooCommerce Shortcuts status ability for authorized automation tools. It reports whether the shortcut data for a selected site is current.

Status lists follow Team Control site access, so they include only sites available to the current user. Before a shortcut opens through Jump to WP Admin, the extension verifies that the target site is still authorized.
