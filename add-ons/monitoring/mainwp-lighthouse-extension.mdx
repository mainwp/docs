...
For legacy REST API v1 requests, use `POST` for `/wp-json/mainwp/v1/lighthouse/audit`. The endpoint no longer accepts `GET` requests.
...
Authorized users can use WordPress Abilities to view Lighthouse observations, audit history, and schedule state for sites they can access. They can request or cancel an audit, and manage Lighthouse policies globally or for an individual site.

Audit operations report whether they are queued, running, completed, or cancelled.
